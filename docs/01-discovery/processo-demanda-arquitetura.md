...
**Nota técnica:** a imagem publicada acima e o arquivo `.bpmn` têm os **mesmos elementos e os mesmos fluxos**, mas layouts diferentes. O arquivo mantém o traçado linear canônico — uma faixa, quatro raias —, que é o que as ferramentas de modelagem esperam ao abrir e editar. A imagem reorganiza o mesmo modelo em três faixas empilhadas, ligadas pelos eventos de link, porque em faixa única o diagrama tem quase 7.000 px de largura e fica ilegível quando embutido em uma página. É a mesma escolha pragmática já feita no [diagrama de implantação](../04-diagramas/README.md): a notação é a mesma, o que muda é o posicionamento — gerado por script, com coordenadas explícitas, em vez de layout automático.
